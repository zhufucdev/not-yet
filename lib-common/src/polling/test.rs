...
#[tokio::test]
#[traced_test]
async fn run_now_notifies_next_reschedule() {
    let s = Arc::new(scheduler());
    let s_detached = s.clone();
    let (read_tx, mut ready_rx) = mpsc::channel(1);
    let coroutine =
        tokio::spawn(async move {
            assert!(s_detached.schedules().await.is_empty());
            ...
            assert!(has_new_queue);
            return future::join_all(s_detached.schedules().await.into_iter().map(
                async |schedule| {
                    let task = Box::pin(s_detached.start_polling(Some(schedule.key())))
                        .next()
                        .await;
                    task.is_some_and(|t| t.is_ok())
                },
            ))
            .await;
        });
    ...
    let schedule = s
        .add_schedule(hourly_interval(), TestKey(2))
        .await
        .expect("valid interval");
    s.run_now(&schedule).await.expect("schedule exists");

    let tasks = timeout(Duration::from_millis(100), coroutine)
        .await
        .expect("timed out on coroutine")
        .expect("coroutine panicked");
    assert!(tasks.iter().all(|it| *it));
    assert!(tasks.len() == 2);
}
